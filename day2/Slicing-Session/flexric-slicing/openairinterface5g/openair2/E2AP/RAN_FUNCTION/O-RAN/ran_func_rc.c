...
//  assert(data->type == RAN_CONTROL_CTRL_V1_03 );
  ...
  if(ctrl->hdr.frmt_1.ric_style_type == 1 && ctrl->hdr.frmt_1.ctrl_act_id == 2) {
  ...
  #ifdef NGRAN_GNB_DU 
  } else if (ctrl->hdr.frmt_1.ric_style_type == 3 && ctrl->hdr.frmt_1.ctrl_act_id == 1) { // HO Slice level
    LOG_I(NR_MAC, "[E2-Agent]: START INTER SLICE HO \n ");

    e2sm_rc_ctrl_msg_frmt_1_t const* msg = &ctrl->msg.frmt_1;
    assert(msg->sz_ran_param == 1 && "not support msg->sz_ran_param != 1");
    seq_ran_param_t* HO_slice_level_msg = &msg->ran_param[0];
    assert(HO_slice_level_msg->ran_param_id == HO_INFO && "wrong HO slice level id");
    assert(HO_slice_level_msg->ran_param_val.type == STRUCTURE_RAN_PARAMETER_VAL_TYPE && "wrong HO slice level type");



    seq_ran_param_t* PLMN_Identity = &HO_slice_level_msg->ran_param_val.strct->ran_param_struct[0];
    assert(PLMN_Identity->ran_param_id == PLMN_Identity_TEMP && "wrong PLMN_Identity id");
    ...
    seq_ran_param_t* S_NSSAI = &HO_slice_level_msg->ran_param_val.strct->ran_param_struct[1];
    assert(S_NSSAI->ran_param_id == DEST_NSSAI_TEMP && "wrong S_NSSAI id");
    ...
    assert(SST->ran_param_id == DEST_SST_TEMP && "wrong SST id");
    ...
    assert(SD->ran_param_id == DEST_SD_TEMP && "wrong SD id");
    ...
    // Just gnb is considered.
    ue_id_e2sm_t ue_id = ctrl->hdr.frmt_1.ue_id; 

    const int mod_id = 0;
    bool rc = rc_HO_slice_level(&ue_id, RC_nssai, mod_id);
    if (!rc)
        LOG_E(NR_MAC, "failed HO slice level\n");

...
/* Optional
  note: this measurement is calculated as per spec */
static meas_record_lst_t fill_Slice_ID(__attribute__((unused))uint32_t gran_period_ms, cudu_ue_info_pair_t ue_info, const size_t ue_idx)
{
  meas_record_lst_t meas_record = {0};
  ...
  // Input: ue_info.ue->slice_list
  // Output: the last slice ID of the UE
  // TODO: Remove this function
  // Receive slice id of the UE for source slice from the RC request. 
  int get_next_if_valid(const NR_list_t *list) {
    assert(list != NULL && "List is NULL");
    assert(list->head >= -1 && "Head is invalid");
    assert(list->len >= 0 && "Length is invalid");
      ...
      while (current_index != -1) {
          ...
          last_value = current_index; // Update the last value to the current index
          current_index = list->next[current_index]; 
      }

      // printf("Last value: %d\n", last_value);
      assert(last_value >= 0 && "Last value is invalid");
      return last_value;
  }


  // Get DL data volume delivered to PDCP layer
  meas_record.int_val = get_next_if_valid(&ue_info.ue->dl_id);
...
static kv_measure_t lst_measure[] = {
  ...
  {.key = "Slice_ID", .value =  fill_Slice_ID }, 
#endif
}; 
...
void print_list(NR_list_t *list) {
  printf("List elements: %d ", list->head);
...
meas_record_lst_t get_kpm_meas_value(char* kpm_meas_name, uint32_t gran_period_ms, cudu_ue_info_pair_t ue_info, const size_t ue_idx)
{
  ...
  // printf("Fetching KPM measurement value for: %s\n", kpm_meas_name);
  // printf("Granularity Period (ms): %u\n", gran_period_ms);
  // printf("Measurement Record fetched for UE Index %zu\n", ue_idx);
  // printf("UE Information:\n");
  // printf("  UE ID: %d\n", ue_info.ue->rnti);     // Replace `ue_id` with the actual field name
  // printf("  RNTI: %d\n", ue_info.ue->uid); 
  // printf("  Assoc slice: \n");
  // print_list(&ue_info.ue->dl_id);
  // if (strcmp(kpm_meas_name, "DRB.UEThpUl") == 0) {
  // printf("  UL Throughput: %u\n", ue_info.ue->ul_thr_ue);
  // }
  // else if  (strcmp(kpm_meas_name, "DRB.UEThpDl") == 0) {
  // printf("  DL Throughput: %u\n", ue_info.ue->dl_thr_ue);
  // }
  // else if  (strcmp(kpm_meas_name, "RRU.PrbTotDl") == 0) {
  // printf("  DL PRB: %u\n", ue_info.ue->mac_stats.dl.total_rbs);
  // }
  // else if  (strcmp(kpm_meas_name, "RRU.PrbTotUl") == 0) {
    // printf("  UL PRB: %u\n", ue_info.ue->mac_stats.ul.total_rbs);
  // }

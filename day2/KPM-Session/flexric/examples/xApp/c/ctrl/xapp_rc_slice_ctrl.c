...
static
e2sm_rc_ctrl_msg_t gen_rc_ctrl_slice_level_PRB_quata_msg(e2sm_rc_ctrl_msg_e msg_frmt)
{
...
// void creat_rc_slice_level_PRB_quotal()
// {
...
//   assert(nodes.len > 0);ue_id_e2sm_t* ue_id, const char* sst_str, const char* sd_str
...
int main(int argc, char *argv[])
{
  ...
  int operation;
  printf("Enter operation (0 for create_slices, 1 for handover): ");
  if (scanf("%d", &operation) != 1 || (operation != 0 && operation != 1)) {
      printf("Invalid input. Please enter 0 (create_slices) or 1 (handover).\n");
      return 1;
  }
